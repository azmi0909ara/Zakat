...
export default function RingkasanPembayaran() {
  ...
  return (
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-100 py-16 px-4">
      <div className="max-w-3xl mx-auto bg-white dark:bg-gray-800 rounded-lg shadow p-8" data-aos="fade-up">
  ...
  {id && (
    <>
      ...
      {id.slice(4)}
              ...
              {data.total && <p><strong>Total Zakat:</strong> {formatRupiah(data.total)}</p>}
              {data.pesan && <p><strong>Pesan/Doa:</strong> {data.pesan}</p>}
            </div>
            ...
            <div className="mt-6 p-4 bg-gray-100 dark:bg-gray-700 rounded">
              <p><strong>Nama:</strong> {rekening[metode].nama}</p>
              <p><strong>Bank / E-Wallet:</strong> {rekening[metode].bank}</p>
              <p><strong>No. Rek / No. HP:</strong> {rekening[metode].norek}</p>
